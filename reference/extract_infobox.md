...
Parses the first \`{{Infobox ...}}\` template from raw wikitext into a
named list of field-value pairs. Uses brace-depth matching to correctly
handle nested templates within field values. Only the first infobox is
extracted if the article contains multiple. Field values are returned
as-is (raw wikitext); use \[clean_infobox_value()\] to strip markup.
